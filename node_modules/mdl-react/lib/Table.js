...
module.exports = React.createClass({
	...
	_perf : function() {
		...
		time += 'ms';
		console.debug('Table._perf ' + time);
	},

	_styleCheckbox : function() {
		if(this.props.selectable) {
			var ths = this.getDOMNode().getElementsByTagName('th');
			if(ths.length > 0) {
				ths[0].style['width'] = '67px';
			}
		}
	},
	...
	render: function() {
		...
		var items = this.props.items.map(function(element, index) {
			var row = this.props.headers.map(function(headerElement, index) {
				if(this.props.itemStyles instanceof Array) {
					return (
						React.createElement("td", {style: this.props.itemStyles[index], key: index}, 
